...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.comuni.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.comunis.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('prefisso') ? 'is-invalid' : '' }}" type="text" name="prefisso" id="prefisso" value="{{ old('prefisso', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('prov') ? 'is-invalid' : '' }}" name="prov_id" id="prov_id">
                    @foreach($provs as $id => $prov)
                        <option value="{{ $id }}" {{ old('prov_id') == $id ? 'selected' : '' }}>{{ $prov }}</option>
                    @endforeach
                </select>
                @if($errors->has('prov'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('stato') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="stato" id="stato" value="1" {{ old('stato', 0) == 1 || old('stato') === null ? 'checked' : '' }}>
