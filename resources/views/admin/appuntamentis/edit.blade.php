...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.appuntamenti.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.appuntamentis.update", [$appuntamenti->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tipologia') ? 'is-invalid' : '' }}" name="tipologia_id" id="tipologia_id" required>
                    @foreach($tipologias as $id => $tipologia)
                        <option value="{{ $id }}" {{ ($appuntamenti->tipologia ? $appuntamenti->tipologia->id : old('tipologia_id')) == $id ? 'selected' : '' }}>{{ $tipologia }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('statoapp') ? 'is-invalid' : '' }}" name="statoapp_id" id="statoapp_id" required>
                    @foreach($statoapps as $id => $statoapp)
                        <option value="{{ $id }}" {{ ($appuntamenti->statoapp ? $appuntamenti->statoapp->id : old('statoapp_id')) == $id ? 'selected' : '' }}>{{ $statoapp }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('filiale') ? 'is-invalid' : '' }}" name="filiale_id" id="filiale_id" required>
                    @foreach($filiales as $id => $filiale)
                        <option value="{{ $id }}" {{ ($appuntamenti->filiale ? $appuntamenti->filiale->id : old('filiale_id')) == $id ? 'selected' : '' }}>{{ $filiale }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('data_ora') ? 'is-invalid' : '' }}" type="text" name="data_ora" id="data_ora" value="{{ old('data_ora', $appuntamenti->data_ora) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('luogo') ? 'is-invalid' : '' }}" type="text" name="luogo" id="luogo" value="{{ old('luogo', $appuntamenti->luogo) }}" required>
                @if($errors->has('luogo'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pratica') ? 'is-invalid' : '' }}" name="pratica_id" id="pratica_id" required>
                    @foreach($praticas as $id => $pratica)
                        <option value="{{ $id }}" {{ ($appuntamenti->pratica ? $appuntamenti->pratica->id : old('pratica_id')) == $id ? 'selected' : '' }}>{{ $pratica }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('richiesta') ? 'is-invalid' : '' }}" name="richiesta_id" id="richiesta_id">
                    @foreach($richiestas as $id => $richiesta)
                        <option value="{{ $id }}" {{ ($appuntamenti->richiesta ? $appuntamenti->richiesta->id : old('richiesta_id')) == $id ? 'selected' : '' }}>{{ $richiesta }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cliente') ? 'is-invalid' : '' }}" name="cliente_id" id="cliente_id" required>
                    @foreach($clientes as $id => $cliente)
                        <option value="{{ $id }}" {{ ($appuntamenti->cliente ? $appuntamenti->cliente->id : old('cliente_id')) == $id ? 'selected' : '' }}>{{ $cliente }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('agentes') ? 'is-invalid' : '' }}" name="agentes[]" id="agentes" multiple required>
                    @foreach($agentes as $id => $agente)
                        <option value="{{ $id }}" {{ (in_array($id, old('agentes', [])) || $appuntamenti->agentes->contains($id)) ? 'selected' : '' }}>{{ $agente }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $appuntamenti->note) }}</textarea>
                @if($errors->has('note'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('durata') ? 'is-invalid' : '' }}" type="number" name="durata" id="durata" value="{{ old('durata', $appuntamenti->durata) }}" step="1" required>
