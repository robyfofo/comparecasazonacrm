    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-alerts.update", [$userAlert->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\UserAlert::TIPO_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('tipo') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="tipo_{{ $key }}" name="tipo" value="{{ $key }}" {{ old('tipo', $userAlert->tipo) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('scadenza_evento') ? 'is-invalid' : '' }}" type="text" name="scadenza_evento" id="scadenza_evento" value="{{ old('scadenza_evento', $userAlert->scadenza_evento) }}">
