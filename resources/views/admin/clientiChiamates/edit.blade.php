...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.clientiChiamate.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.clienti-chiamates.update", [$clientiChiamate->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="data_ora">{{ trans('cruds.clientiChiamate.fields.data_ora') }}</label>
                <input class="form-control datetime {{ $errors->has('data_ora') ? 'is-invalid' : '' }}" type="text" name="data_ora" id="data_ora" value="{{ old('data_ora', $clientiChiamate->data_ora) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('filiale') ? 'is-invalid' : '' }}" name="filiale_id" id="filiale_id">
                    @foreach($filiales as $id => $filiale)
                        <option value="{{ $id }}" {{ ($clientiChiamate->filiale ? $clientiChiamate->filiale->id : old('filiale_id')) == $id ? 'selected' : '' }}>{{ $filiale }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('agente') ? 'is-invalid' : '' }}" name="agente_id" id="agente_id" required>
                    @foreach($agentes as $id => $agente)
                        <option value="{{ $id }}" {{ ($clientiChiamate->agente ? $clientiChiamate->agente->id : old('agente_id')) == $id ? 'selected' : '' }}>{{ $agente }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cliente') ? 'is-invalid' : '' }}" name="cliente_id" id="cliente_id">
                    @foreach($clientes as $id => $cliente)
                        <option value="{{ $id }}" {{ ($clientiChiamate->cliente ? $clientiChiamate->cliente->id : old('cliente_id')) == $id ? 'selected' : '' }}>{{ $cliente }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pratica') ? 'is-invalid' : '' }}" name="pratica_id" id="pratica_id">
                    @foreach($praticas as $id => $pratica)
                        <option value="{{ $id }}" {{ ($clientiChiamate->pratica ? $clientiChiamate->pratica->id : old('pratica_id')) == $id ? 'selected' : '' }}>{{ $pratica }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('richiesta') ? 'is-invalid' : '' }}" name="richiesta_id" id="richiesta_id">
                    @foreach($richiestas as $id => $richiesta)
                        <option value="{{ $id }}" {{ ($clientiChiamate->richiesta ? $clientiChiamate->richiesta->id : old('richiesta_id')) == $id ? 'selected' : '' }}>{{ $richiesta }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('testo') ? 'is-invalid' : '' }}" name="testo" id="testo">{!! old('testo', $clientiChiamate->testo) !!}</textarea>
                @if($errors->has('testo'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('stato') ? 'is-invalid' : '' }}" type="number" name="stato" id="stato" value="{{ old('stato', $clientiChiamate->stato) }}" step="1">
                @if($errors->has('stato'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('direzione') ? 'is-invalid' : '' }}" name="direzione" id="direzione" required>
                    ...
                    @foreach(App\ClientiChiamate::DIREZIONE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('direzione', $clientiChiamate->direzione) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nome') ? 'is-invalid' : '' }}" type="text" name="nome" id="nome" value="{{ old('nome', $clientiChiamate->nome) }}">
                @if($errors->has('nome'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cognome') ? 'is-invalid' : '' }}" type="text" name="cognome" id="cognome" value="{{ old('cognome', $clientiChiamate->cognome) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('telefono') ? 'is-invalid' : '' }}" type="text" name="telefono" id="telefono" value="{{ old('telefono', $clientiChiamate->telefono) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cellulare') ? 'is-invalid' : '' }}" type="text" name="cellulare" id="cellulare" value="{{ old('cellulare', $clientiChiamate->cellulare) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="text" name="email" id="email" value="{{ old('email', $clientiChiamate->email) }}">
                @if($errors->has('email'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('data_risposta') ? 'is-invalid' : '' }}" type="text" name="data_risposta" id="data_risposta" value="{{ old('data_risposta', $clientiChiamate->data_risposta) }}">
