...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.calTagAppuntamentoAgenti.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.cal-tag-appuntamento-agentis.update", [$calTagAppuntamentoAgenti->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="label">{{ trans('cruds.calTagAppuntamentoAgenti.fields.label') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('label') ? 'is-invalid' : '' }}" name="label" id="label">{!! old('label', $calTagAppuntamentoAgenti->label) !!}</textarea>
                @if($errors->has('label'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('appuntamento') ? 'is-invalid' : '' }}" name="appuntamento_id" id="appuntamento_id" required>
                    @foreach($appuntamentos as $id => $appuntamento)
                        <option value="{{ $id }}" {{ ($calTagAppuntamentoAgenti->appuntamento ? $calTagAppuntamentoAgenti->appuntamento->id : old('appuntamento_id')) == $id ? 'selected' : '' }}>{{ $appuntamento }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('agente') ? 'is-invalid' : '' }}" name="agente_id" id="agente_id" required>
                    @foreach($agentes as $id => $agente)
                        <option value="{{ $id }}" {{ ($calTagAppuntamentoAgenti->agente ? $calTagAppuntamentoAgenti->agente->id : old('agente_id')) == $id ? 'selected' : '' }}>{{ $agente }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('data_ora') ? 'is-invalid' : '' }}" type="text" name="data_ora" id="data_ora" value="{{ old('data_ora', $calTagAppuntamentoAgenti->data_ora) }}" required>
