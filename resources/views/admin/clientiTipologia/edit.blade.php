...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.clientiTipologium.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.clienti-tipologia.update", [$clientiTipologium->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="tipologia">{{ trans('cruds.clientiTipologium.fields.tipologia') }}</label>
                <input class="form-control {{ $errors->has('tipologia') ? 'is-invalid' : '' }}" type="text" name="tipologia" id="tipologia" value="{{ old('tipologia', $clientiTipologium->tipologia) }}" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('stato') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="stato" id="stato" value="1" {{ $clientiTipologium->stato || old('stato', 0) === 1 ? 'checked' : '' }} required>
